...
export async function handler(event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> {
    ...
    if(event.resource === "/products"){
        ...
        if(event.httpMethod === "PUT"){
            ...
            } catch (ConditionalChackFailedException) {
                console.error(ConditionalChackFailedException)
                return { 
                    statusCode: 404, 
            ...
            try {
                ...
                const response = await sendProductEvent(product, ProductEventType.DELETED, "no-reply@raphaelamericano.com.br", lambdaRequestId)
                console.log(response)
                return {
                    statusCode: 200,
...
function sendProductEvent(product: Product, eventType: ProductEventType, email: string, lambdaRequestId: string ){
    ...
    return lambdaClient.invoke({
        ...
        InvocationType: "Event" // " sync => RequestResponse"
    }).promise()
}