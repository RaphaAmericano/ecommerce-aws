...
export class OrderEventRepository{
    ...
    public createOrderEvent(orderEvent: OrderEventDdb){
    ...
    public async getOrderEventsByEmail(email: string){
    ...
    public async getOrderEventsByEmailAndEventType(email: string, eventType: string){
        const data = await this.ddbClient.query({
            TableName: this.eventsDdb,
            IndexName: "emailIndex",
            KeyConditionExpression: 'email = :email AND begins_with(sk, :prefix)',
