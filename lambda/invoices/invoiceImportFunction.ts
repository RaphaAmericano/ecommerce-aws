...
import { InvoiceFile } from "./layers/invoiceRepository/nodejs/invoiceRepository"
...
async function processRecord(record: S3EventRecord){
    ...
    try {
        ...
        if(invoice.invoiceNumber.length >= 5){
            ...
            await Promise.all([updateInvoicePromise, sendStatusPromise, disconnectClient, putEventPromise])
